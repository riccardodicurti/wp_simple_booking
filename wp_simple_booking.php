...
function wp_sb_get_dependencies() {
	...
	$options['license_code'] = apply_filters( 'wp_simple_booking/wp_sb_get_dependencies/default_license_code_0', $wordpress_simple_booking_options['default_license_code_0'] );
	...
	$options['prenota_url'] = wp_sb_get_sburl_by_license_code( $wordpress_simple_booking_options['default_license_code_0'], $options['language_code'] ); 
	$options['richiedi_url'] = wp_sb_get_richiediurl_by_license_code( $wordpress_simple_booking_options['request_page_id'], $options['language_code'] ); 
	$options['prenota_label'] = __( 'Prenota', 'wp_sb' ); 
	$options['richiedi_label'] = __( 'Richiedi', 'wp_sb' );

	return $options;
}

function wp_sb_enqueue_dependencies() {
	$options = wp_sb_get_dependencies();

	if ( $options['license_code'] != '0000' ) {
		wp_enqueue_style( 'wp_sb_style', plugin_dir_url( __FILE__ ) . 'public/css/simple_booking_style.css' );
		wp_register_script( 'wp_sb_scripts', plugin_dir_url( __FILE__ ) . 'public/js/simplebooking.js', [ 'jquery' ], false, true );

		wp_localize_script( 'wp_sb_scripts', 'options', $options );
		wp_enqueue_script( 'wp_sb_scripts' );
	}
}

function wp_sb_admin_error_notice() {
	global $pagenow;
	...
	if ( $pagenow == 'admin.php' && isset( $_GET['page'] ) && htmlspecialchars( $_GET['page'] ) == 'wordpress-simple-booking' ) {
		...
		printf( '<div class="notice error my-acf-notice is-dismissible"><p>' . __( 'Unlock more "WordPress Simple Booking" features installing <a href="%s">Advanced Custom Fields</a>', 'wp_sb' ) . '.</p></div>', $install_acf_url );
	}
}

function wp_sb_init() {
	add_action( 'wp_enqueue_scripts', 'wp_sb_enqueue_dependencies' );
	...
	if ( ! is_plugin_active( 'advanced-custom-fields/acf.php' ) && 
		...
		if ( is_admin() ) {
			add_action( 'admin_notices', 'wp_sb_admin_error_notice' );
		}
	} else {
		add_action( 'acf/init', 'wp_sb_acf_init' );
	}
}

add_action( 'init', 'wp_sb_init' );
